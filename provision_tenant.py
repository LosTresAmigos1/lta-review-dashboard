...
def _build_initial_artifacts(approved_locations: list[dict]) -> dict[str, bytes]:
    ...
    # Multi-Tenant Phase 4P: the canonical, collision-safe slug -- a real
    # multi-location tenant can legitimately approve several identically-
    # titled locations (different addresses/googleLocationIds); a bare
    # slugify(title) would collide two of them into the SAME
    # reviews/by-location/{slug}.json artifacts dict key, silently
    # dropping one location's placeholder file before Initial Sync ever
    # runs. See db.canonical_location_slugs().
    titles_by_id = {
        loc["locationId"]: (loc.get("title") or f"Location {loc['locationId']}")
        for loc in approved_locations
    }
    slug_map = db.canonical_location_slugs(titles_by_id)

    loc_list = []
    artifacts: dict[str, bytes] = {}
    for loc in sorted(approved_locations, key=lambda l: l.get("title") or ""):
        title = titles_by_id[loc["locationId"]]
        slug = slug_map[loc["locationId"]]
